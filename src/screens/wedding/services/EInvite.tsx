...
const InviteCard = React.memo(({ item, index, scrollY }: { item: any; index: number; scrollY: any }) => {
    ...
    const handlePress = () => {
        let screenName = 'routes/InviteStudioScreen';
        switch (item.title) {
            case 'Haldi':
                screenName = 'routes/HaldiInviteScreen';
                break;
            case 'Sangeet':
                screenName = 'routes/SangitInviteScreen';
                break;
            case 'Mehndi':
                screenName = 'routes/MehndiInviteScreen';
                break;
            case 'Wedding':
                screenName = 'routes/WeddingInviteScreen';
                break;
            case 'Reception':
                screenName = 'routes/ReceptionInviteScreen';
                break;
            default:
                screenName = 'routes/InviteStudioScreen';
                break;
        }
        router.push({ pathname: screenName as any, params: { eventType: item.title } });
    };
            ...
            <View style={styles.container}>
                ...
                <Animated.FlatList
                    ...
                    renderItem={({ item, index }) => (
                        <InviteCard item={item} index={index} scrollY={scrollY} />
                    )}
                    numColumns={2}
